...
";
        ...
        public void It_Should_Be_Possible_To_Save_And_Load_Cloud_Service_Archive()
        {
            ...
            cloudServiceArchive.AddToscaServiceTemplate("tosca.yaml",
                new ToscaServiceTemplate {ToscaDefinitionsVersion = "tosca_simple_yaml_1_0"});
        ...
        public void TraverseNodeTypesByRequirements_Traverses_Nodes_From_Specific_Node_Type_By_It_Requirements()
        {
            ...
            powerNodeType.AddRequirement("power.cable",
                new ToscaRequirement {Node = "tosca.nodes.cable", Capability = "cable"});
            powerNodeType.AddRequirement("power.switch",
                new ToscaRequirement {Node = "tosca.nodes.switch", Capability = "cable"});

            var deviceNodeType = new ToscaNodeType();
            deviceNodeType.AddRequirement("power",
                new ToscaRequirement {Node = "tosca.nodes.power", Capability = "power"});
            ...
            var validationErrors = string.Join(Environment.NewLine,
                validationResults.Select(a => a.ErrorMessage).ToArray());
            validationErrors.Should().BeEmpty();

            // Act
            var discoveredNodeTypeNames = new List<string>();
            cloudServiceArchive.TraverseNodeTypesByRequirements("tosca.nodes.device",
                (nodeTypeName, nodeType) => { discoveredNodeTypeNames.Add(nodeTypeName); });

            // Assert
            discoveredNodeTypeNames.ShouldBeEquivalentTo(new[]
            {"tosca.nodes.device", "tosca.nodes.power", "tosca.nodes.switch", "tosca.nodes.cable"});
        }
